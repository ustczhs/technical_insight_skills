...
通过 grilling 把产品概念收敛成可筛选主控硅片的结构化输入（Selection Brief），再由技术端产出 SoC Shortlist。v1 内置两个 Product Family：陪伴机器人、可穿戴 AI。落地为两个 skill：`hardware-selection-brief`（Phase 1）与 `soc-shortlist`（Phase 2）。
...
**Evidence Grade**:
Phase 2 对公开信息的来源等级，与 `ai_hardware_insight` 对齐：A（官方/拆解/认证库）> B（权威媒体/供应商）> C（二手信息）。每条用于筛选的关键规格须在文档中标注等级与来源；不得编造。引用格式：`[A/B/C] 内容（信息时效：YYYY-MM，来源 URL）`。
_Avoid_: 可信度（口语）, 星级

**Evidence Freshness**:
用于描述现状时的时效规则：超过 48 个月的 [A] 源降为 [B]，直至有更新近源印证。阈值相对 `ai_hardware_insight` 的 24 个月放宽，以适配长生命周期芯片。
