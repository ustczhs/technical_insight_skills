...
| **0b 澄清/压测** | agent 判断清晰度：模糊 → full grill（≤8 轮）；清晰 → 简流程（1–3 轮）；含假设压测。**全部为带推荐的选择题** | 交错更新两文件草稿 |
| **0c 结构化采集** | **一次一题** `AskQuestion`（或同等选择题）收集基本参数，禁止一张表连发多题 | 更新 `调研基调.md` |
| **0d 定稿闸门** | 展示摘要；用户确认后设 `Step 0 状态：已确认` | 勾选进度检查点 |

**交互纪律**（详见 intent-discovery）：一次一题、选择题+推荐+末项「其他」；事实 agent 自查、决策必问（Lazy 除外）；共识前不进入 Step 1（Lazy 除外）。
...
- `成功标准` / `明确不做`（可短）
- 基本参数表：调研目的、深度、范围、读者、硬件扩展包、必证项、**分析师立场**、我方主体（第一方必填）、**交互模式**（`逐步确认` / `Lazy`）
...
- **闸门**（逐步确认）：用户确认模板后进入 Step 2。**Lazy**：采用推荐裁剪，自勾 `Step 1 模板确认`，不暂停。
...
- **闸门**（逐步确认）：用户确认列表后进入 Step 3。**Lazy**：采用推荐分层，自勾 `Step 2 竞品列表确认`，不暂停。
